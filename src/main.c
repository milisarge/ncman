...
static struct {
    ...
    }, // CONTEXT_SERVICE_CONFIG_STANDALONE
};
...
static void action_on_signal(struct json_object *jobj) {
    ...
    is_tech_removed = strncmp(key_sig_tech_removed,
                              json_object_get_string(sig_name), 50) == 0;
    if (context.tech->dbus_name == NULL) {
        is_current_tech = false;
    } else {
        is_current_tech = strncmp(context.tech->dbus_name,
                                  json_object_to_json_string(sig_data), 256) == 0;
    }
...
static void modify_service_config(void) {
    ...
    for (i = 0; main_fields[i]; i++) {
        ...
        if (pos_label == 0) {
            ...
            if (strncmp(key_serv_autoconnect, field_buffer(main_fields[i - 1], 0),
                        11) != 0) {
                continue;
            }
